...
galaxy_ml_dir = str(pathlib.Path(__file__).resolve().parents[1])
...
def class_to_source_link(cls):
    ...
    link = ('https://github.com/goeckslab/'
            'Galaxy-ML/blob/master/' + path + '#L' + str(line))
    return '[[source]](' + link + ')'
...
def process_list_block(docstring, starting_point, section_end,
    ...
    top_level_regex = r'^    ([^\s\\\(]+) :(.*)'
    top_level_replacement = r'- **\1**:\2<br>'
...
def read_page_data(page_data, type):
    ...
    for module in page_data.get('all_module_{}'.format(type), []):
        ...
        for name in dir(module):
            ...
            if (
                inspect.isclass(module_member) and type == 'classes'
...
def generate(sources_dir):
    ...
    """
    template_dir = os.path.join(galaxy_ml_dir, 'docs', 'templates')
    ...
    readme = read_file(os.path.join(galaxy_ml_dir, 'README.md'))
    index = read_file(os.path.join(template_dir, 'index.md'))
    index = index.replace('{{autogenerated}}', readme[readme.find('#'):])
    with open(os.path.join(sources_dir, 'index.md'), 'w') as f:
        f.write(index)

    print('Generating docs for Galaxy-ML %s.' % galaxy_ml.__version__)
    for page_data in PAGES:
        ...
        if not blocks:
            raise RuntimeError(
                'Found no content for page ' + page_data['page'])
    ...
    shutil.copyfile(os.path.join(galaxy_ml_dir, 'docs', 'CHANGELOG.md'),
                    os.path.join(sources_dir, 'CHANGELOG.md'))
    """copy_examples(os.path.join(galaxy_ml_dir, 'examples'),
                  os.path.join(str(sources_dir), 'examples'))"""


if __name__ == '__main__':
    generate(os.path.join(galaxy_ml_dir, 'docs', 'sources'))
